...
func _spawn_and_start() -> void:
	...
	if customerNum == 5:
		...
		await tw.finished
		var msg := "I am landlord give $%s pls" % [str(Global.landlordRequirement)]
	...
	msg = msg.replace("ADJ", Global.adjectives[Global.noun_key])
	await customer.type_text(msg)
...
func try_to_sell(customer: CharacterBody3D) -> void:
	if Global.landlord == true:
		if Global.money >= Global.landlordRequirement:
			...
			Global.landlordRequirement *= 2
			customer.queue_free()
			Global.landlord = false
			_spawn_and_start()
		else:
			var msg := "Wait!!! Your poor twin! ♥🥀"
			await customer.type_text(msg)
	elif (Global.noun_key == Global.onTable):
		var findIndex = 0
		for i in get_tree().get_nodes_in_group("Antique"):
